...
from django.db.models import Sum, Avg, F, ExpressionWrapper, FloatField , Count
...
class GoalRepository:
    ...
    @staticmethod
    def get_stats_for_each_player_in_each_match():
        return (
            goal.objects.values('player', 'match_id')
            .annotate(goal_count=Count('player') , assist_count = Count('assist'))
            .order_by('player', 'match_id')
        )
...
class MatchRepository:
    ...
    def get_possession_stats():
        return match_info.objects.values('match_id' , 'home_team', 'away_team')\
    .annotate(home_possession = F('home_team_possession') , away_possession = F('away_team_possession'))\
    .order_by('match_id')
...
class PlayerRepository:
    ...
    @staticmethod
    def create(player_name, date_of_birth, place_of_birth, height , nationality, position, current_club, sign_contract_date, contract_expired , age , price):
        return player.objects.create(
            ...
            sign_contract_date=sign_contract_date,
